...
async def test_login_user(client):
    response = await client.post("/api/auth/login", data={"username": test_user["name"], "password": test_user["password"]})
    assert response.status_code == 200
    assert "access_token" in response.json()

    response = await client.post("/api/auth/login", data={"username": "wronguser", "password": "wrongpass"})
...
async def test_confirm_email(client):
    ...
    response = await client.get(f"/api/auth/confirm_email/{valid_token}")
    assert response.status_code == 201
    ...
    response = await client.get(f"/api/auth/confirm_email/{invalid_token}")
